...
# 通信频率设定为 10Hz (每 24 个物理 step 通信一次)
COMM_FREQ_STEPS = 24
# 0.1秒(10Hz)内，Franka机械臂单关节最大物理转动约为 0.2 弧度
MAX_DELTA = 0.2
...
if HAS_RL_LIBS:
    class BitAllocationEnv(gym.Env):
        ...
        def step(self, action):
            ...
            reward = np.clip(improvement_mm, -100.0, 100.0)
        ...
        for traj_idx, traj in enumerate(trajectories):
            ...
            def move_to_10hz(target_pos, duration, record, is_settle=False):
                ...
                for step_idx in range(steps):
                    if is_settle:
                        # 【稳态评估核武】：稳定期目标不再移动，消除一切物理惯性延迟，只暴露纯量化噪声！
