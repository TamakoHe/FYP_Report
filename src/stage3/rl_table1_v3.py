...
if HAS_RL_LIBS:
    ...
    # ==========================================
    # 【核心升级：TqdmCB 收敛追踪回调器 (输出 Reward-Step)】
    # ==========================================
    class TqdmCB(BaseCallback):
        def __init__(self, steps, eval_freq=10000): 
            super().__init__()
            self.pbar = None
            self.steps = steps
        ...
        def _on_training_start(self): 
            self.pbar = tqdm(total=self.steps, desc="🏎️ 3D感知深度淬炼与收敛监控")

        def _on_step(self): 
            self.pbar.update(self.locals.get("env").num_envs)
            
            # 每一物理步，收集智能体获得的真实 Reward
            rewards = self.locals.get("rewards")
            if rewards is not None:
                self.reward_buffer.append(np.mean(rewards))
            ...
            if self.num_timesteps - self.last_eval_step >= self.eval_freq:
                ...
                # 计算过去一万步的平均 Reward
                mean_reward = np.mean(self.reward_buffer) if len(self.reward_buffer) > 0 else 0.0
                self.reward_buffer = []  
                
                # 暂停一下，跑一趟真实轨迹，收集当前的真实任务表现
                metrics = run_evaluation_trajectory(self.model, TOTAL_ARM_BITS, gui=False)
                
                # 提取稳态的 Grasp 和 Place 误差
                err_grasp_rl = metrics['rl']['grasp'] * 1000
                err_place_rl = metrics['rl']['place'] * 1000
                
                # 记录到历史数据中
                self.eval_history.append((self.num_timesteps, mean_reward, err_grasp_rl, err_place_rl))
                
                # 实时更新进度条后缀
                self.pbar.set_postfix({"Reward": f"{mean_reward:.2f}", "Grasp(mm)": f"{err_grasp_rl:.2f}"})
                
                # 控制台播报
                tqdm.write(f"📈 [Step {self.num_timesteps:>6}] 收敛评估 | 均值 Reward: {mean_reward:>6.2f} | Grasp误差: {err_grasp_rl:>6.2f}mm")
        ...
        def _save_data(self):
            """使用 Pandas 进行安全、规范的数据落盘"""
            if not self.eval_history:
                return
            import pandas as pd
            file_name = f"training_convergence_{TOTAL_ARM_BITS}bit.csv"
            # 获取绝对路径，方便用户寻找
            abs_path = os.path.abspath(file_name)
            
            df = pd.DataFrame(self.eval_history, columns=["Step", "Mean_Reward", "Grasp_Error_mm", "Place_Error_mm"])
...
def run_evaluation_trajectory(rl_model, total_bits, gui=False):
    ...
    # 为了加快训练时的后台评估，如果 rl_model 不为空且不显示 GUI，仅跑 RL 方案
    schemes_to_run = ['rl'] if (not gui and rl_model is not None) else ['avg', 'static', 'rl']
...
if __name__ == '__main__':
    ...
    if HAS_RL_LIBS:
        if os.path.exists(MODEL_PATH):
            ...
            # 【设定回调频率】: 30万步训练，每 10000 步评估一次，恰好能拿到 30 个极其清晰的收敛坐标点！
